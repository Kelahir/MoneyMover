...
## How to use
MoneyMover is split into two classes:
- `MoneyMover` - the underlying logic (login, wallets, bank statement parsing, preset matching, adding transactions). No terminal prompts or printing.
- `MoneyMoverCLI` - a thin wrapper around a `MoneyMover` instance that adds the interactive terminal prompts and printed reports.

### 1. Connect to the MoneLover API
Import both classes with `from money_mover import MoneyMover, MoneyMoverCLI`, create a `MoneyMover` instance, then wrap it: `cli = MoneyMoverCLI(ml)`. You will need to supply `MoneyMover` with your MoneyLover email and password. There are three ways to pass them:
...
- `ml.wallets` attribute returns your wallet summary
- `cli.set_wallet()` prompts you to choose an active wallet for other methods, like:
    - `ml.this_month` and `ml.previous_month` attributes return records for the set wallet for the current and previous months respectively.
    - `ml.request_transactions()` allows you to show records for the custom range of dates.
    - `cli.display_categories()` shows an interactive menu with all available category names.
...
Now you can also run `cli.print_bank_report()` to compare transactions in the bank statement with the ones you already have in the MoneyLover for that time period. Neat!

You can already run `cli.transfer_bank_transactions()` and add the transactions one by one manually.
> **Note:**
...
Now you can display your presets using `cli.print_user_presets()` method. `cli.print_bank_report()` will also highlight matches for you! Running a `cli.transfer_bank_transactions()` will show you matches with your presets and offer to populate all these records for you.
